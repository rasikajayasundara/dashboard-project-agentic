...
// GET responses return display strings ("Billable", "Fixed Price"); the form
// and PUT body need the "1"/"2" value convention used across this feature.
...
export default function EditProject({ project = {}, onClose }) {
  ...
  const INITIAL_VALUES = {
    ...
    feeType:          mapBillingTypeToValue(project.billingType),
    officeLocation:   project.locationId        ?? "",
    marketSegment:    project.marketSegment    || "",
    projectType:      mapProjectTypeToValue(project.projectType),
  ...
  const { isLoading: isSaving, savedAt } = editProject || {};
  ...
  function handleSubmit() {
    ...
    dispatch(projectAction.updateProjectStart({
      ...
      billingType:      isBillable ? parseInt(values.feeType) : parseInt(HOURLY_FEE),
